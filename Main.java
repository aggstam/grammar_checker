...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Stack<stackNode> st= new Stack<stackNode>(); // Head of stack.
        st.add(new stackNode(word));
        
        while(!st.isEmpty()){                        // While the stack is not empty
            stackNode current = st.pop();            // Get the stack head.
            if(isSolution(current.word,userWord)){  // Check if it's a solution...
                System.out.println("HOORAY!! The word can be produced by the specified Grammar.");
                break;
            }
            else{
                generateChilder(grammar,st,current,userWord); // ..else generate it's children using the Grammar's rules.
            }
        }
    ...
    public static void generateChilder(Grammar grammar,Stack st,stackNode current, char[] userWord){
        for(int i=0;i<current.word.length;i++){
            for(int j=0;j<grammar.rules.length;j++){
                if(grammar.rules[j].producer==current.word[i]){
                    ...
                    if(finalChars(grammar,temp)<=userWord.length){ // Child's word must not have more terminal symbols than the word's length we're testing.
    ...
    public static char[] readFromUser(){
        ...
        while(Sword.isEmpty()){
            ...
            System.out.println();
            System.out.println("Enter a word:");
            Sword=keyboard.nextLine();
        }
        
        char[] word=new char[Sword.length()];
        for(int i=0;i<word.length;i++){
            word[i]=Sword.charAt(i);
        }
        return word;
    }
